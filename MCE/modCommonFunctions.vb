...
Public Function ConvertFromUnixTimeStamp(ByVal dblTimeStamp As Double, ByVal blnConvertToLocalTime As Boolean) As DateTime
'---------------------------------------------------------------------
'Purpose     : Converting UNIX time stamp (http://en.wikipedia.org/wiki/Unix_time) to .NET's compliant Date and Time
'Assumptions :
'Input       :
'   - dblTimeStamp, a value with Double type containing the unix time stamp
'   - blnConvertToLocalTime, a boolean indicating whether to convert the result into local time zone
'Returns     :
'   - a .NET compliant DateTime object
'Note        : 
'   - Initially after convertion the date and time value is based on UTC (http://en.wikipedia.org/wiki/Utc)
'-------------------------------------------------------------------------------
  On Error GoTo ErrorHandler

  Dim dtBaseUnixTime As DateTime  


  'Create the base of UNIX time
  dtBaseUnixTime = New DateTime(1970, 1, 1, 0, 0, 0, 0)

  'Add seconds since that time
  If blnConvertToLocalTime = True Then
    ConvertFromUnixTimeStamp = dtBaseUnixTime.AddSeconds(dblTimeStamp).ToLocalTime
  Else
    ConvertFromUnixTimeStamp = dtBaseUnixTime.AddSeconds(dblTimeStamp)
  End If

  Exit Function
ErrorHandler:
  GenericErrorHandler("modCommonFunctions.vb", "ConvertFromUnixTimeStamp", Err.Number, Err.Source, Err.Description)
  ConvertFromUnixTimeStamp = New DateTime(1970, 1, 1, 0, 0, 0, 0)
End Function
